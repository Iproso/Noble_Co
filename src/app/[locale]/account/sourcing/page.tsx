...
import { SourcingRequest } from '@/components/features/member';
import { mockSourcingRequests } from '@/lib/mock/member-data';
...
export default async function SourcingPage({ params }: Props) {
  ...
  return (
    <MemberShell locale={locale} title={t('title')}>
      ...
      {mockSourcingRequests.length > 0 ? (
        <div className="space-y-3">
          {mockSourcingRequests.map((req) => (
            <SourcingRequest key={req.id} locale={locale} request={req} />
          ))}
        </div>
      ) : (
        <div className="text-center py-16">
          <p className="text-sm text-text-secondary mb-4">{t('noRequests')}</p>
          <CTAButton locale={locale} variant="outline">{t('submitRequest')}</CTAButton>
        </div>
      )}
    </MemberShell>
  );
