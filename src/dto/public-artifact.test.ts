...
describe('PublicArtifactDetailDTOSchema', () => {
  it('validates minimal detail', () => {
    const result = PublicArtifactDetailDTOSchema.safeParse({
      ...
      yearPeriod: null, maison: null, maker: null,
      provenanceSummary: null, conditionSummary: null,
      evidenceLabel: null, confidenceLabel: null,
      passportAvailable: false, passportPreview: null,
      mediaGallery: [], costClarity: null,
      viewingAvailability: null, riskTier: 1,
    });
    expect(result.success).toBe(true);
  });
});
